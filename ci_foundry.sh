...
warn_path_conflict() {
  ...
  if [ -n "$which_path" ] && [ "$which_path" != "$bin_path" ]; then
    warn "Multiple binaries with the name '$bin' are present in your 'PATH'."
    warn "You may need to run 'rm $which_path' or move '$FOUNDRY_BIN_DIR' in your 'PATH' to prioritize the newly installed version!"
  fi
}
